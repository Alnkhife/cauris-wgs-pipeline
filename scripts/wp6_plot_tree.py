...
TREE = '$HOME/Projects/cauris-wgs-analysis/analysis/07_context/tree_combined.contree'
OUT_PDF = '$HOME/Projects/cauris-wgs-analysis/output/Figure_WP6_combined_tree.pdf'
OUT_PNG = '$HOME/Projects/cauris-wgs-analysis/output/Figure_WP6_combined_tree.png'
...
color = {}
for k in K143B: color[k] = '#1f77b4'
for k in K143A: color[k] = '#5a8fbf'
...
def draw(nod, px):
    ...
    if ch is None:
        py = y[name]
        x = px + dist
        ax.plot([px, x], [py, py], 'k-', lw=0.7)
        ax.scatter([x], [py], color=color.get(name, 'gray'), s=8, zorder=5)
        fs = 5.5 if name.startswith(('ERR', 'SRR')) else 7
    ...
    ax.plot([x, x], [ymin, ymax], 'k-', lw=0.7)
    if label:
        ax.text(x + 0.001, (ymin + ymax) / 2, label, fontsize=5, color='0.5')
...
leg = [Line2D([0], [0], marker='s', color='w', markerfacecolor='#1f77b4', markersize=8, label='Study: K143R/CDR1_V704L (clade B)'),
       Line2D([0], [0], marker='s', color='w', markerfacecolor='#5a8fbf', markersize=8, label='Study: K143R/CDR1_V704L (clade A)'),
...
ax.legend(handles=leg, loc='upper left', fontsize=7, frameon=False)
